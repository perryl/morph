...
class InitiatorConnection(distbuild.StateMachine):
    ...
    def _handle_list_requests(self, event):
        ...
        output_msg = []
        output_msg.append('%s distbuild build-request(s) currently in progress'
                          % len(jobs))
        for build in requests:
            output_msg.append('Initiator connection (address:port): %s\n'
                              'Build request message: %s\nBuild request ID: %s'
                              % (build.get_init_conn().initiator_name,
                              build.get_request(), build.get_request()['id']))
